			...
			if	it[0] != am.NODATA_value and\
				...
				rg.region[vert3] != am.NODATA_value:
					file.write('\tboxToCell\n\t{\n\t\tbox (%lf %lf %lf) (%lf %lf %lf);\n\t\tfieldValues\n\t\t(\n\t\t\tvolScalarFieldValue alpha.water 1\n\t\t);\n\t}\n' % (\
						it.multi_index[0] * am.dx, it.multi_index[1] * am.dx ,am.altitude[it.multi_index] -  am.alt_min + indent,\
...
def createSetFieldsFourPhasesSoilRotated(am, rg, height = 2, height_soil = 5): #rg - region map, height - height of snow cover
	print("Creating setFieldsDict file.")
	file = open("setFieldsDict", "w")
	file.write('FoamFile\n{\n\tversion\t2.0;\n\tformat\tascii;\n\tclass\tdictionary;\n\tlocation\t"system";\n\tobject\tsetFieldsDict;\n}\n')
	file.write('defaultFieldValues\n(\n\tvolScalarFieldValue alpha.gas 1\n\tvolScalarFieldValue alpha.liquid 0\n\tvolScalarFieldValue alpha.soil 0\n\tvolScalarFieldValue alpha.entrained 0\n);\nregions\n(\n')
	indent = 0
	soil_indent = am.dx * math.ceil(height_soil/am.dx)
	with np.nditer(rg.region, flags=['multi_index'], op_flags=['readonly']) as it:
		while not it.finished:
			vert0 = it.multi_index
			vert1 = tuple(map(add, it.multi_index, (1, 0)))
			vert2 = tuple(map(add, it.multi_index, (1, 1)))
			vert3 = tuple(map(add, it.multi_index, (0, 1)))
			if	am.altitude[vert0] != am.NODATA_value and\
				it.multi_index[0]+1 < am.nx and\
				it.multi_index[1]+1 < am.ny and\
				am.altitude[vert1] != am.NODATA_value and\
				am.altitude[vert2] != am.NODATA_value and\
				am.altitude[vert3] != am.NODATA_value:
					if	it[0] != am.NODATA_value and\
						rg.region[vert1] != am.NODATA_value and\
						rg.region[vert2] != am.NODATA_value and\
						rg.region[vert3] != am.NODATA_value:
							file.write('\trotatedBoxToCell\n\t{\n\t\torigin (%f %f %f);\n\t\ti (%f %f %f);\n\t\tj (%f %f %f);\n\t\tk (%f %f %f);\n\t\tfieldValues\n\t\t(\n\t\t\tvolScalarFieldValue alpha.liquid 1\n\t\t\tvolScalarFieldValue alpha.soil 0\n\t\t\tvolScalarFieldValue alpha.entrained 0\n\t\t\tvolScalarFieldValue alpha.gas 0\n\t\t);\n\t}\n' % (\
								it.multi_index[0] * am.dx, it.multi_index[1] * am.dx, am.altitude[vert0] - am.alt_min + indent + soil_indent,\
								am.dx, 0, am.altitude[vert1] - am.altitude[vert0],\
								0, am.dx, am.altitude[vert3] - am.altitude[vert0],\
								0, 0, height))
					else:
